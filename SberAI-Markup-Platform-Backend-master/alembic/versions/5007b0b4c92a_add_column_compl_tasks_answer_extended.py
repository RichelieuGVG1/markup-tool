...
def upgrade() -> None:
    ...
    """)
    op.execute("""
           ALTER TABLE `completed_tasks` CHANGE COLUMN `answer` `answer` MEDIUMTEXT NOT NULL COLLATE 'utf8mb4_general_ci' AFTER `task`;
    """)
...
def downgrade() -> None:
    ...
    """)
    op.execute("""
        ALTER TABLE `completed_tasks` CHANGE COLUMN `answer` `answer` TEXT NOT NULL COLLATE 'utf8mb4_general_ci' AFTER `task`;
    """)
